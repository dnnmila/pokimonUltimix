...
// ── Battle Royal ────────────────────────────────────────────────────────────
// Los rivales los sortea el backend a partir del color de token de cada Pokémon
// vivo del jugador, así que arrancar no necesita nada más que quién lo lanza.
// El rival de cada combate lo pone el propio `-round` al cerrar el anterior.
const royaleStart = (playerId) => raidPost('royale-start', { playerId });
const royaleRound = (playerId, hostTotal, rivalTotal) => raidPost('royale-round', { playerId, hostTotal, rivalTotal });
const royaleClear = (playerId) => raidPost('royale-clear', { playerId });
