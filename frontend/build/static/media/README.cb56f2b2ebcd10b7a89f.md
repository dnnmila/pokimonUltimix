...
Mientras un retrato no exista, la pantalla cae automáticamente al token de carta
de `images/Leaders<numero>/` — el que ya trae al líder junto a su Pokémon.

Los usan la pantalla del jugador (`SimPlayer`) y el modal de batalla del máster
(`ModalBattle`), los dos por `src/data/leaders.js`.
...
### Alto Mando, Campeón y Rival

El modal de batalla del máster los pinta con la misma tarjeta, así que aceptan
retrato igual que los gimnasios — hoy ninguno lo tiene y salen con su carta.
Misma regla de nombre: `agatha`, `bruno`, `lorelei`, `lance`, `teamrocket`,
`gary`. El Campeón de Kanto ya usa `blue`, y los tres equipos de Blue comparten
retrato (en pantalla se numeran Blue 1, Blue 2, Blue 3).

Los rivales por color (`RivPink1`, `RivBlue2`…) se llaman todos "Gary": un solo
`gary` los cubre, porque el color de la casilla ya va escrito en la tarjeta.
